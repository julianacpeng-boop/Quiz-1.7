...
def criar_clipe_imagem(img_path, duracao, out_path, audio_path=None):
    # IMPORTANTE:
    # todos os inputs precisam entrar antes das opções de saída do FFmpeg.
    cmd = [
        "ffmpeg", "-y",
        "-loop", "1",
        "-i", str(img_path),
    ...
    if audio_path:
        # completa o restante da duração com silêncio;
        # assim a contagem e as pausas não são cortadas pelo áudio curto.
        cmd += [
            "-af", "apad",
            "-c:a", "aac",
            "-b:a", "192k",
...
def criar_clipe_animado(frames_dir, fps_anim, duracao, out_path, audio_path=None):
    ...
    if audio_path:
        # mantém a duração completa do clipe, incluindo a pausa programada.
        cmd += [
            "-af", "apad",
            "-c:a", "aac",
            "-b:a", "192k",
